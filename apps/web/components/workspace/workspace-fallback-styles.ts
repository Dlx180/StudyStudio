export const WORKSPACE_FALLBACK_STYLES = `
  ...
  * { box-sizing: border-box; }
  ...
  .reader-header, .reader-context-actions, .dock-panel-header { display: flex; align-items: flex-start; justify-content: space-between; gap: 1rem; }
  ...
  .dock-toggle, .reader-context-actions button, .selection-action-bar button, .page-controls button, .dock-panel-header, .concept-chip, .concept-tree-node button, .visual-actions button { border: 1px solid #cbd8ea; border-radius: 8px; background: #fff; color: #172033; cursor: pointer; }
  ...
  .unit-node small, .dock-panel-header small, .visual-header span, .console-prompt, .muted { color: #60718f; font-size: .85rem; }
  ...
  .interaction-console { grid-template-rows: minmax(0, 1fr) auto; height: 100%; }
  ...
  .concept-pool, .concept-draft { min-height: 0; overflow: auto; border: 1px solid #dbe4f0; border-radius: 10px; background: #fff; padding: .75rem; }
  ...
  .console-output.state { color: #e4dcff; }
  .console-output.next { color: #ffe5bf; }
